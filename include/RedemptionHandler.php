...
class RedemptionHandler
{
	...
	public function get_all_items($Company_id) 
	{
        ...
        $stmt = $this->conn->prepare("SELECT * FROM igain_company_merchandise_catalogue WHERE Company_id = ? and Valid_from <= ? and Valid_till >= ? and show_item = 1 and Active_flag = 1 and Billing_price != 0 and Billing_price_in_points != 0 and Ecommerce_flag = 0"); 
	...
	public function get_all_items_filter($Company_id,$filtervalue) 
	{
		$Todays_date=date("Y-m-d");
        $stmt = $this->conn->prepare("SELECT i.* FROM igain_company_merchandise_catalogue as i JOIN igain_merchandize_category as c ON c.Merchandize_category_id = i.Merchandize_category_id WHERE i.Company_id = ? and i.Valid_from <= ? and i.Valid_till >= ? and i.show_item = 1 and i.Active_flag = 1 and i.Billing_price != 0 and i.Billing_price_in_points != 0 and i.Ecommerce_flag = 0 and (c.Merchandize_category_name LIKE '%".$filtervalue."%' OR i.Merchandize_item_name LIKE '%".$filtervalue."%')"); 
